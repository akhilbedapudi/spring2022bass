...
n_subjects = 204 # more subjects requires more memory
...
#treatmentsort = treatment.iloc[0:203:3]

###############################################################################
# Sex is encoded as 'M' or 'F'. Hence, we make it a binary variable.
genotype = mouse_database[3]
genotypestring = genotype.to_string()
#genotypesort = genotype.iloc[0:203:3]
...
design_matrix = pd.DataFrame(np.vstack((treatment, genotype, intercept)).T,
                             columns=['treatment', 'genotype', 'intercept'])
...
plotting.plot_stat_map(
    z_map, bg_img=mask_img, threshold=threshold, colorbar=True,
